...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, object_angvel: torch.Tensor, object_linvel: torch.Tensor, progress_buf: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Adaptive angular velocity target based on orientation error
    # When orientation error is small (close to target), target lower angular velocity
    # When orientation error is large (far from target), allow higher angular velocity
