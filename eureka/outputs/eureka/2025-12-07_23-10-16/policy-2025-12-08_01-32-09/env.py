...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, object_angvel: torch.Tensor, object_linvel: torch.Tensor, progress_buf: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # When orientation error is high (far from target), allow higher angular velocity
    # When orientation error is low (close to target), encourage lower angular velocity to prevent overshooting
    angvel_magnitude = torch.norm(object_angvel, dim=-1)
    target_angvel = 2.0 * orientation_error  # Scale target angular velocity with orientation error
    
    # Temperature parameters for reward shaping
    orientation_temp = 10.0
    linvel_temp = 2.0
    angvel_temp = 1.0  # Increased temperature for more precise angular velocity control
