...
class ShadowHandQwen(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_rot, self.goal_rot, self.object_angvel, self.object_linvel, self.progress_buf)
...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, object_angvel: torch.Tensor, object_linvel: torch.Tensor, progress_buf: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Angular velocity - adaptive target based on orientation error
    # When close to target (low orientation_error), we want very low angular velocity
    # When far from target (high orientation_error), we can allow higher angular velocity
    angvel_magnitude = torch.norm(object_angvel, dim=-1)
    
    # Adaptive target angular velocity: decreases as we get closer to target
    # Max target when orientation_error is 1.0 (completely misaligned): ~2.0
    # Min target when orientation_error is 0.0 (perfectly aligned): ~0.1
    max_target_angvel = 2.0
    min_target_angvel = 0.1
    target_angvel = min_target_angvel + (max_target_angvel - min_target_angvel) * orientation_error
    
    # Temperature parameters for reward shaping
    orientation_temp = 10.0
    linvel_temp = 2.0
    angvel_temp = 1.0  # Increased temperature for better fine control
    
    # Reward components
    orientation_reward = torch.exp(-orientation_temp * orientation_error)
    linvel_reward = torch.exp(-linvel_temp * linvel_penalty)
    angvel_reward = torch.exp(-angvel_temp * torch.abs(angvel_magnitude - target_angvel))
