...
def compute_reward(
    ...
    # --- Adaptive angular velocity reward (combining Code A's proximity logic with Code B's target magnitude) ---
    angvel_norm = torch.norm(object_angvel, dim=-1)
    proximity_threshold = 0.1
    close_to_target = orientation_error <= proximity_threshold

    # When close to target: strongly penalize any angular motion to encourage stillness
    angvel_penalty_close = torch.exp(-5.0 * angvel_norm)

    # When far from target: encourage moderate spinning toward the target (target ~2.0 rad/s as in both parents)
    angvel_temp_far = 0.5
