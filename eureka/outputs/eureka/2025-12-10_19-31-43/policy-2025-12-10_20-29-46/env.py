...
class ShadowHandQwen(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.object_rot, self.goal_pos, self.goal_rot, self.object_linvel, self.object_angvel, self.actions, self.fingertip_pos)
...
def compute_reward(
    ...
    # Distance to goal position (to encourage keeping object near target location)
    ...
    rot_angle = 2.0 * torch.asin(torch.clamp(torch.norm(quat_diff[:, 1:], p=2, dim=-1), max=1.0))
    rot_temp = 5.0
    rot_reward = torch.exp(-rot_temp * rot_angle)

    # Object retention: encourage fingertips to stay near object
    fingertip_object_dist = torch.norm(fingertip_pos - object_pos.unsqueeze(1), p=2, dim=-1)
    mean_fingertip_dist = torch.mean(fingertip_object_dist, dim=1)
    retention_temp = 5.0
    retention_reward = torch.exp(-retention_temp * mean_fingertip_dist)

    # Small penalty for excessive action magnitude
    action_penalty = -0.01 * torch.sum(actions ** 2, dim=-1)

    # Small penalty for large linear velocity (to reduce flinging)
    linvel_penalty = -0.01 * torch.norm(object_linvel, p=2, dim=-1)

    # Combine rewards
    total_reward = (
        pos_reward +
        rot_reward +
        0.5 * retention_reward +
        action_penalty +
        linvel_penalty
    )
