...
def compute_reward(
    ...
    # For unit quaternions, the vector part (x,y,z) is sin(theta/2) * axis
    ...
    target_angvel_mag = 4.0  # higher target to ensure visible spinning
    ...
    # Stronger action regularization (but not too strong)
    action_penalty = torch.sum(actions ** 2, dim=-1)
    action_reg_weight = 0.0005
    action_reward = -action_reg_weight * action_penalty

    # Very light joint velocity penalty to reduce jitter (not prevent motion)
