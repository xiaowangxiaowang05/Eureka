...
def compute_reward(
    ...
    rot_reward = torch.exp(-1.5 * rot_angle)
    ...
    angvel_reward = torch.exp(-0.8 * angvel_error)
    
    # Action regularization (very light)
    action_penalty = torch.sum(actions ** 2, dim=-1)
    action_reward = -0.0002 * action_penalty

    # Joint velocity penalty (lighter)
    joint_vel_reward = -5e-7 * torch.sum(shadow_hand_dof_vel ** 2, dim=-1)
    
    # Encourage stable grasp: assume object_pos z < 0.5 indicates drop (since hand starts at z=0.5)
    # Since object_pos isn't available, approximate via low angvel + high rot error + no spin
    frozen = (torch.norm(object_angvel, dim=-1) < 0.5) & (rot_angle > 1.0)
    drop_penalty = torch.where(frozen, -2.0, torch.zeros_like(rot_angle))
