...
def compute_reward(
    ...
    rot_angle = 2.0 * torch.acos(torch.abs(w))  # absolute angle error in [0, pi]
    
    # Temperature for orientation reward shaping
    rot_temp = 1.0
    rot_reward = torch.exp(-rot_temp * rot_angle)
    
    # Encourage moderate angular velocity to maintain spin (not too slow, not too fast)
    ...
    # Joint velocity regularization: penalize very high joint velocities
