...
class ShadowHandQwen(VecTask):
    ...
    def compute_reward(self, actions):
        ...
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_rot, self.goal_rot, self.object_angvel, self.actions, self.shadow_hand_dof_vel)
...
def compute_reward(
    ...
    # The rotation error is measured as the angle between object and goal orientations
    quat_error = quat_mul(object_rot, quat_conjugate(goal_rot))
    # Extract w component (real part); angle = 2 * acos(|w|)
    w = quat_error[:, 3]
    w = torch.clamp(w, -1.0 + 1e-6, 1.0 - 1e-6)
    rot_angle_error = 2.0 * torch.acos(torch.abs(w))
    
    # Temperature for orientation reward shaping
    orientation_temp = 1.0
    orientation_reward = torch.exp(-orientation_temp * rot_angle_error)

    # Encourage appropriate angular velocity for spinning
    # Assume that spinning requires non-zero angular velocity; penalize being too slow
    angvel_norm = torch.norm(object_angvel, dim=-1)
    angvel_temp = 0.5
    angvel_reward = torch.exp(-angvel_temp * torch.abs(angvel_norm - 2.0))  # Target ~2 rad/s

    # Action regularization to minimize jitter and energy use
    action_penalty = torch.sum(actions ** 2, dim=-1)
    action_temp = 0.01
    action_reward = -action_temp * action_penalty

    # Joint velocity regularization to prevent shaking
