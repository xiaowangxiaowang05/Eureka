...
class ShadowHandQwen(VecTask):
    ...
    def compute_reward(self, actions):
        ...
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_rot, self.goal_rot, self.object_angvel, self.actions, self.fingertip_pos, self.object_pos, self.progress_buf, self.max_episode_length, self.shadow_hand_dof_pos, self.shadow_hand_dof_vel, self.shadow_hand_dof_default_pos, self.vec_sensor_tensor)
...
def compute_reward(
    ...
    # Penalty for deviation from neutral hand pose (encourage natural posture)
    hand_pose_error = torch.norm(shadow_hand_dof_pos - shadow_hand_dof_default_pos, dim=-1)
    pose_temp = 0.5
    pose_penalty = -pose_temp * hand_pose_error

    # Penalty for joint acceleration (reduce jitter)
    # Approximate acceleration as change in velocity (assuming dt=1 for relative comparison)
    # Since we don't have previous velocity, use current velocity magnitude as proxy for excessive motion
    vel_norm = torch.norm(shadow_hand_dof_vel, dim=-1)
    accel_temp = 0.05
    accel_penalty = -accel_temp * vel_norm

    # Penalty for excessive fingertip forces (prevent overcompensation)
    # vec_sensor_tensor shape: [num_envs, num_fingertips * 6] (force + torque per fingertip)
    force_components = vec_sensor_tensor[:, :num_fingertips*3].view(-1, num_fingertips, 3)
    avg_force_magnitude = torch.mean(torch.norm(force_components, dim=-1), dim=1)
    force_temp = 0.02
    force_penalty = -force_temp * avg_force_magnitude
