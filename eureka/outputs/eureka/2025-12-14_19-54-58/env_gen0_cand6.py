...
class ShadowHandQwen(VecTask):
    ...
    def compute_reward(self, actions):
        ...
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_rot, self.goal_rot, self.object_angvel, self.actions, self.fingertip_pos, self.object_pos, self.progress_buf, self.max_episode_length)
...
def compute_reward(
    ...
    # Distance in SO(3): d = 1 - |w|, but we use 1 - w^2 for smoothness
    ...
    # Fingertip dispersion: encourage fingers to stay around object
    # Assume object is near origin; compute average fingertip distance to object
    ...
    # Ideal distance depends on object size; assume ~0.05m for small object
    ideal_fingertip_dist = 0.08
    fingertip_error = torch.abs(avg_fingertip_dist - ideal_fingertip_dist)
    fingertip_temp = 5.0
    fingertip_reward = torch.exp(-fingertip_temp * fingertip_error)

    # Time-based survival bonus to encourage longer episodes (helps learning)
