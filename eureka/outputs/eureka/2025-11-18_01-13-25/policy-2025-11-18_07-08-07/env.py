...
def compute_reward(root_states: torch.Tensor, up_vec: torch.Tensor, actions: torch.Tensor, dt: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Action penalty for energy efficiency - reformulated to be more sensitive
    action_norm = torch.sum(actions * actions, dim=-1)
    
    # Temperature parameters for proper balance
    speed_temp = 1.0        # Direct reward for speed (no transformation needed)
    ...
    action_penalty = -action_temp * action_norm  # Direct penalty scaled by temperature
    
    # Combine rewards
    reward = speed_reward + stability_penalty + action_penalty
