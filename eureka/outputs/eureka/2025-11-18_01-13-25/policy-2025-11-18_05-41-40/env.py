...
class HumanoidQwen(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.up_vec, self.actions, self.dt)
...
def compute_reward(root_states: torch.Tensor, up_vec: torch.Tensor, actions: torch.Tensor, dt: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Use direct quadratic penalty for better sensitivity to small deviations
    up_deviation = 1.0 - up_vec[:, 2]  # Positive when leaning away from upright
    stability_penalty_raw = -torch.square(up_deviation) * 10.0  # Scale to provide meaningful penalty
    ...
    speed_temp = 0.1        # Lower temperature to prevent speed reward from dominating too much
    stability_temp = 1.0    # No exponential needed, using direct penalty
    ...
    stability_penalty = stability_penalty_raw  # Direct penalty without exponential
