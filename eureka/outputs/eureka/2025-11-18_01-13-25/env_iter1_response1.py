...
def compute_reward(root_states: torch.Tensor, up_vec: torch.Tensor, actions: torch.Tensor, dt: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Temperature parameters for reward shaping - adjusted for better balance
    speed_temp = 0.1        # Reduced from 1.0 to prevent dominance
    stability_temp = 2.0    # Increased from 0.5 to make stability more impactful
    action_temp = 0.05      # Increased from 0.01 for better regularization
