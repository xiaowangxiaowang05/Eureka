...
def compute_reward(root_states: torch.Tensor, up_vec: torch.Tensor, actions: torch.Tensor, dt: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Action penalty for energy efficiency (linear penalty is more effective than exponential here)
    action_penalty = -torch.sum(actions * actions, dim=-1)
    
    # Temperature parameters for proper balance
    speed_temp = 0.1        # Keep lower temperature to prevent excessive dominance
    stability_temp = 8.0    # Increase temperature to make stability penalty more impactful
    action_temp = 0.05      # Increase temperature for more effective action regularization
    ...
    action_penalty_shaped = action_temp * action_penalty  # Linear scaling instead of exponential
